...
$(document).ready(function () {
    ...
    $("input.digeradres").each(function (i) {
        $(this).change(function () {
            ...
            for (var i = 0; i < inputid.length - 1; i++) {
                if (i == 0) {
                    inputclientid += inputid[i].toString();
                } else {
                    inputclientid += '_' + inputid[i].toString();
                }

            }

            inputid = inputid[inputid.length - 1];
            var kat = ' Kat:';
            var pk = ' PK:';

            if ($('#' + inputclientid + '_txtKatBilgisi').val() != "") {
                kat = ' Kat:';
            } else {
                kat = ' ';
            }
            ...
            if ($('#' + inputclientid + '_txtPostaKodu').val() != "") {
                pk = ' PK:';
            } else {
                pk = ' ';
            }
            ...
            if (inputid == "txtDaireNo") {
                var deger = $('#' + inputclientid + '_txtKapiNo').val() + '/' + $(this).val() + kat + $('#' + inputclientid + '_txtKatBilgisi').val() + pk + $('#' + inputclientid + '_txtPostaKodu').val();
                $('#' + $(this).data("digeradres")).val(deger);
                $('#' + inputclientid + '_hdfEkAciklama').html(deger);
            }
            if (inputid == "txtKapiNo") {
                var deger = $(this).val() + '/' + $('#' + inputclientid + '_txtDaireNo').val() + kat + $('#' + inputclientid + '_txtKatBilgisi').val() + pk + $('#' + inputclientid + '_txtPostaKodu').val();
                $('#' + $(this).data("digeradres")).val(deger);
                $('#' + inputclientid + '_hdfEkAciklama').html(deger);
            }

            if (inputid == "txtKatBilgisi") {
                var deger = $('#' + inputclientid + '_txtKapiNo').val() + '/' + $('#' + inputclientid + '_txtDaireNo').val() + kat + $(this).val() + pk + $('#' + inputclientid + '_txtPostaKodu').val();
                $('#' + $(this).data("digeradres")).val(deger);
                $('#' + inputclientid + '_hdfEkAciklama').html(deger);
            }

            if (inputid == "txtPostaKodu") {
                var deger = $('#' + inputclientid + '_txtKapiNo').val() + '/' + $('#' + inputclientid + '_txtDaireNo').val() + kat + $('#' + inputclientid + '_txtKatBilgisi').val() + pk + $('#' + inputclientid + '_txtPostaKodu').val();
                $('#' + $(this).data("digeradres")).val(deger);
                $('#' + inputclientid + '_hdfEkAciklama').html(deger);
            }
    ...
    $("input.autoComplete").each(function (i) {
        var a = this;
        var fonksiyon = "";
        if ($(a).data("fonksiyon") != undefined)
            fonksiyon = $(a).data("fonksiyon").toString();

        if ($(a).data("ustclientid") != undefined && $(a).data("ustclientid") != '') {
            var ust = $(a).data("ustclientid");
            var hiddenvalue = $("#" + ust).attr("data-hdfclientid");
        ...
        //var altInputlar = function (altclientid) {
        //    var altinput = $("#" + $(a).data("altclientid"));
        //    if (altinput.data("altclientid") != undefined) {
        //        $('#' + altinput.data("altclientid")).val((ui.item ? ui.item.id : ""));
        //        $('#' + altinput.data("altclientid")).attr('readonly', 'readonly').addClass('ac_locked');
        //        $('#' + altinput.data("altclientid")).attr('data-deger', '');
        //        $('#' + altinput.data("altclientid")).attr('data-val', '');
        //        altInputlar(altinput.data("altclientid"));
        //    }
        //}
        ...
        $(this).autocomplete({
            ...
            source: function (request, response) {
                debugger;
                if (ust) {
                    var jsonData = "{ 'prefix': '" + request.term + "', 'ustid': '" + $("#" + hiddenvalue).val() + "'}";
                } else {
                    var jsonData = "{ 'prefix': '" + request.term + "'}";
                }


                $.ajax({
                    url: '../WebServisler/AutoCompleteGenel.asmx/' + fonksiyon,
                    data: jsonData,
                    dataType: "json",
                    type: "POST",
                    contentType: "application/json; charset=utf-8",
                    success: function (data) {
                        var veri = data.d.split('#');
                        veri.pop();
                        response($.map(veri, function (item) {
                            return {
                                label: item.split('-')[1],
                                val: item.split('-')[0],
                                kod: item.split('-')[2]
            ...
            select: function (event, ui) {
                ...
                if ($(a).data("val") != ui.item.val) {
                    if ($(a).data("altclientid") != undefined) {
                        $("#" + $(a).data("altclientid")).val((ui.item ? ui.item.id : ""));
                        $("#" + $(a).data("altclientid")).attr('readonly', 'readonly').addClass('ac_locked');
                        $("#" + $(a).data("altclientid")).attr('data-deger', '');
                        $("#" + $(a).data("altclientid")).attr('data-val', '');
                        $('#' + $(a).data("adres")).html('');

                        $('#' + $(a).data("hdfclientid")).val('');
                        ...
                        if (altinput.data("altclientid") != undefined) {
                            ...
                            $('#' + altinput.data("altclientid")).attr('data-deger', '');
                            $('#' + altinput.data("altclientid")).attr('data-val', '');
                            $('#' + altinput.data("adres")).html('');
                            $('#' + altinput.data("hdfclientid")).val('');

                            altinput = $("#" + $(altinput).data("altclientid"))
                            if (altinput.data("altclientid") != undefined) {
                                $('#' + altinput.data("altclientid")).val((ui.item ? ui.item.id : ""));
                                $('#' + altinput.data("altclientid")).attr('readonly', 'readonly').addClass('ac_locked');
                                $('#' + altinput.data("altclientid")).attr('data-deger', '');
                ...
                //label ve değerler hidden ve valulara atandı
                var value = ui.item.val;
                value = value.split(',');
                $(a).attr('data-val', value[value.length - 1]);
                $('#' + $(a).data("hdfclientid")).val(value[value.length - 1]);
                $('#' + $(a).data("adres")).html(ui.item.value);

                $(a).attr('data-deger', ui.item.value);
                //$("#" + $(a).data("deger")).val(ui.item.value);


                if ($(a).data("altclientid") != undefined && $(a).data("clientid") != '') {
                    $("#" + $(a).data("altclientid")).removeAttr("readonly")
                }
            ...
            change: function (event, ui) {
                ...
                if (!$('#' + inputclientid + '_txtBina').val()) {
                    $('#' + inputclientid + '_txtKapiNo').val('');
                    $('#' + inputclientid + '_txtKatBilgisi').val('');
                    $('#' + inputclientid + '_txtPostaKodu').val('');
                    $('#' + inputclientid + '_txtDigerAdresBlg').val('');
                    $('#' + inputclientid + '_hdfEkAciklama').html('');
                }

                if (!$('#' + inputclientid + '_txtIcKapi').val()) {

                    $('#' + inputclientid + '_txtDaireNo').val('');
                    $('#' + inputclientid + '_txtKatBilgisi').val('');
                    $('#' + inputclientid + '_txtPostaKodu').val('');
                    $('#' + inputclientid + '_txtDigerAdresBlg').val('');
                    $('#' + inputclientid + '_hdfEkAciklama').html('');
                ...
                if ($(a).data("altinput")) {
                    ...
                    var inputid = $(a).data("altinput");
                    inputid = inputid.split('_');

                    if (inputid[inputid.length - 1] == "txtKapiNo") {
                        $('#' + $(a).data("altinput")).val(ui.item.value.substring(4));
                        var digeradres = $('#' + $(a).data("altinput")).data("digeradres")
                        $('#' + $(a).data("altinput")).trigger("change");
                    }

                    if (inputid[inputid.length - 1] == "txtDaireNo") {
                        $('#' + $(a).data("altinput")).val(ui.item.value.substring(7));
                        var digeradres = $('#' + $(a).data("altinput")).data("digeradres")
                        $('#' + $(a).data("altinput")).trigger("change");
                    }
    ...
    if ($('#dialog').length) {
        ...
        if ($('#dialog').attr("target") == "") {
            ...
            $('#dialog').dialog({
                ...
                position: { my: "center", at: "center", of: "#" + $('#dialog').attr("target") }
            });
        }
        ...
        $('.inputDialog').each(function (index) {
            ...
            if ($(this).attr("target") == "") {
                ...
                $(this).dialog({
                    ...
                    position: { my: "center", at: "center", of: "#" + $(this).attr("target") }
                }).parent().appendTo($("form"));
            }
